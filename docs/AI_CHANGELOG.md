...
## 2026-09-01 (Claude Code) Routineの重複整理と月曜レポートへの統合
- Routine 14件のプロンプトを全件突き合わせ、重複を整理した（→12件）。
- 「デイリー監視と司令塔は重複か」への答え：**重複していない**。デイリー監視は判断して手を動かす側、
  司令塔は状態を写すだけ（プロンプトに「作業はせず」と明記）。重なるのはデータ取得だけ。
- 削除：`週次：進行ボードの棚卸し`（司令塔とANDSYNC分が重複／memoryツール依存で空振りの疑い）、
  `案件インテーク自動リサーチ（デバイス紐付け版）`（同名タスクの旧版・停止済み）。
- 月曜朝に4本積み上がっていたのを1本へ。司令塔を月曜除外（`0 22 * * 1-6`）、
  週次経営レビューを「月曜レポート」に改称して JST月曜7:37 へ前倒し（`30 22 * * 0`）、
  本文を定型4ブロックに固定して進行ボードの役割を吸収した。
- 主な変更ファイル：`docs/ops-change-log.md`
- 確認結果：`list_triggers` で司令塔の次回発火が 09/03(木) 07:00 JST、
  月曜レポートが 09/07(月) 07:37 JST になっていることを確認した。
- 未対応・次の作業：**広告デイリー監視のプロンプトが古いガードレール値を抱えている**
