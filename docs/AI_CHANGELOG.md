...
## 2026-09-06 — 8月〜9月上旬の棚卸しと、zenryoku-facts の確定値の修正

### 背景
オーナーから「8/1から現在までの状況を分析し、広告費が見合っているのか、
今の計画の損益分岐点は超えられるのか」の依頼。windsor-data（30日）・kpi-data・
freee実績ベースのDriveノートを突合して棚卸しした。

### 分かったこと
- **広告費は予算超過していない。** 実測 Meta+Google ¥360,184/30日（日¥12,006）に対し、
  2026-08-07改定の上限は日¥12,000。8/23ノートと8/31週次レビューの「超過」判定は、
  改定前の日¥11,000を分母にした誤り。
- **損益分岐点は4段で、①②は届き、④（2027年2月〜）と月商300万は現体制では届かない。**
  時間あたり売上¥5,834 × 容量400時間＝理論最大月商¥2,333,614。
  ④は必要施術時間454h、300万は514hで、いずれも容量400hを超える。
- **広告の損益分岐は「回数券の45%以上を広告が生んでいるか」。**
  広告費¥360,184 ÷ 限界利益率0.922 ＝ ¥390,655 ＝ 回数券10.3本。月間実績22.8本に対し45%。
  8/24週の来店経路×販売データでは、広告帰属の上限が25%（1本/4本）で下回っている。
  ただしn=4かつ、/lp の予約ボタンがHPB送客リンクのため広告経由とHPB自然流入を分離できない。

### 変更したファイル
- `.claude/skills/zenryoku-facts/SKILL.md`
  - 広告上限を日¥11,000 → **日¥12,000（Meta 6,000＋Google 6,000）** へ修正
  - 回数券平均単価を推定「約52,000〜54,000円」→ **実数 ¥37,928** へ差し替え
  - 損益分岐を「約253.9万円」単一値 → **freee実績ベースの4段** へ差し替え
  - 時間あたり売上・客単価・理論最大月商の行を追加

### 成果物
- Drive 00_Inbox `2026-09-06_【棚卸し】8月〜9月上旬の実績と損益分岐点は超えられるのか.md`
  （薬機法チェック `scripts/yakkihou-ng.mjs` 通過・終了コード0）

### 未対応
- 広告の帰属計測（受付の「きっかけ」項目追加、LP専用HPBクーポンの作成）はオーナー作業
- 8/1〜8/6のMeta/Google内訳は未取得（Windsorの取得窓が30日）
- HPB掲載料が広告宣伝費に含まれるかは未確定
- 8/3・8/10・8/17週の売上に払い戻し未反映がないかは未確認

## 2026-09-06（追記）— 広告宣伝費をfreee実額で分解、HPB混入の死角を解消

### 背景
「今までの客入りと回数券の販売率を加味して広告費を絞れないか」の検討依頼。

### 分かったこと
- **8月の広告宣伝費¥379,527が、検算一致で完全に分解できた。**
  Meta ¥164,659（隔日¥14,969の閾値課金×11）＋ Google ¥167,451（閾値課金4件）
  ＋ **HPB掲載料 ¥45,100（推定・毎月同額の銀行振込）** ＋ その他¥2,317。
  → `zenryoku-facts` の死角「HPB掲載料が広告宣伝費に混ざっているか未確定」は
  **混ざっていた**という結論で解消。
- **HPBの新規獲得単価は¥1,049/新規で全チャネル最安**（¥45,100 ÷ 月43人）。切る対象ではない。
- 広告流入の予約アクションが91→163（+79%）になった週でも新規来店は17→22（+5人）。
  買っているクリックの大半が来店に変換されていない。
- Meta クリエイティブ別（30日）：B_golf_image は CPA ¥2,233 で
  D_hatsujoriku_banner（¥284）の7.9倍。Meta予算の43%を使ってリードの13%しか出していない。

### 変更したファイル
- `.claude/skills/zenryoku-facts/SKILL.md`
  - 広告宣伝費の内訳（Meta/Google/HPBの実額分解）を追加
  - HPBの新規獲得単価¥1,049の行を追加

### 成果物
- Drive 00_Inbox `2026-09-06_広告費をどこまで絞れるか_freee実額の分解と削減シナリオ.md`

### 提案（未実行・承認待ち）
- B_golf_image と C_hq_75off_4900 の停止（Metaのクリエイティブ変更は自動実行禁止）
- Google 日¥6,000→¥4,000（下げる方向はガードレール上は事前承認済みだが、今回は承認を待つ）
- 削減計 月−¥152,347。損益分岐点① ¥2,190,475 → ¥2,025,240、必要な上積みは回数券 月8.7本 → 4.4本
- 損益中立ライン：新規来店が月18人（現在の19%）以上減るなら損

### 未対応
- HPB掲載料¥45,100は摘要が空欄のため金額・周期からの推定。請求書での確認が必要
- 広告の帰属計測（受付項目の追加、LP専用HPBクーポン）は未着手

## 2026-09-06（訂正）— 8月の人件費は分析に入っていなかった

### 背景
オーナーから「この分析って8月の実際の給与も加味できてる？」の確認。
検証したところ**入っていなかった**ため、損益分岐点を引き直す。

### 分かったこと
- **freeeの給与は支払日ベースで1か月ズレている。**
  7/15払い（7名へ個別振込8件）¥824,550＝**6月分**、8/14払い（一括1件）¥1,181,473＝**7月分**。
  8月分は9/15支給予定で、**freee上にまだ存在しない**。
  つまり「freeeの8月の給料手当」は7月分であり、8月の労働に対応する人件費ではない。
- モデルが使っていた人件費「約¥930,000」は実績ではなく、8/22時点の382.9時間から
  月末換算した**推計**。しかも回数券バック7名全員0本（計上漏れ疑い・7月は24本¥45,000）と
  労働保険が抜けている。
- 引き直すと 損益分岐点① は **¥2,190,475 → ¥2,245,383**（8月シフト維持）。
  7月シフトに戻すと **¥2,515,728** で、必要施術時間431hが容量400hを超え**構造的に届かない**。
- 含意：①に届くのは「8月のシフト水準のまま稼働率を48%→75%へ上げる」場合に限られる。
  シフトを増やす話ではない。
- **freee上の8月は締まっていない**（地代家賃・法定福利費・保険料が¥0）。
  また通信費がモデル想定¥40,127に対し実額¥137,017（7月は¥375,193）で、経常なら
  分岐点がさらに約¥105,000上がる。要確認。

### 変更したファイル
- `.claude/skills/zenryoku-facts/SKILL.md`
  - 損益分岐点を「暫定」と明示し、8月シフト維持／7月シフト復帰の2本立てへ
  - 分岐点が暫定である理由（給与1ヶ月ズレ・推計値・抜け項目）を追記
  - 人件費の内訳（総支給・回数券バック・社保・労働保険）を追加
  - 稼働率48%→75%の行を追加

### 成果物
- Drive 00_Inbox `2026-09-06_【訂正】8月の人件費は分析に入っていない_給与1ヶ月ズレと損益分岐点の引き直し.md`

### 未対応
- 8月分給与（9/15支給）の確定額取り込み ← これが済むまで分岐点は暫定
- 8月の回数券バック0本が正しいか給与システムで確認（8/23から継続・最優先）
- 通信費の中身確認（月¥137,000が経常か）
- 8月の締め（地代家賃・法定福利費・保険料の計上）
...
## 2026-08-31 (Claude Code) 経路アンケートの回答を確認。想定した場所と違ったのでパーサーを修正
- 同日15:21の予約（BF60199476・回数券のリピーター）で**アンケート回答が通知メールに載ることを確認**
- 🔴 **ただし載る場所が想定と違った。** 数時間前に「`■ご要望・ご相談` に載る」と想定して実装し、
  `metrics-definitions.md` にもそう書いてテストまで置いていたが、**実データで否定された**。
  `■ご要望・ご相談` は `-` のままで、回答は **`■サロンからお客様への質問` ブロック末尾の「回答：」行**
- 修正：`parseSurveyAnswer()` を追加し、`surveyAnswer`（生の回答）と `surveyChannel`（1〜6）を返す。
  番号以外の自由記述は `surveyChannel` に落とさない。日別の経路内訳（`byChannel`）も集計に追加
- 誤った前提のテストを実データ準拠の3件に差し替え。テストは13件（既存と合わせて62件）すべてPASS。
  実物のメール本文でCLIの出力も確認（【1】ホットペッパーで検索 1件）
- ⚠️ **設問は「最初に知ったきっかけ」。今回の予約経路ではない。**
  HPB上で予約している人が【1】と答えるのは自然なので、【1】の多さを「HPB経由が多い」と読まない。
  広告の効果を見るのは【2】【3】の増減。`metrics-definitions.md` に明記した
- **今日の「予約ゼロ」は解消。** アンケートが表示された状態で予約が成立しており、
  ここで完全にシロ。15時までの0件は月曜の下振れだった

## 2026-08-31 (Claude Code) 経路アンケートの集計から既存客を外した（オーナー指摘）
- オーナー指摘：**先の1通目（BF60199476）は回数券保有者がHPBから予約したもので、新規の来店経路ではない。**
  さらに**公式LINE →「予約方法」バナー → ネット予約 → HPBへ遷移**という経路が実在し、
  この人はHPB上で予約するため【1】ホットペッパーで検索を選びやすい
- 🔴 **私の実装は全回答を経路に数えていた。** 既存客を分母に入れると広告の評価が壊れる
- 修正：`classifyCustomer()` を追加し `customerType` を `first` / `repeat` / `unknown` の3値で持つ
  （`first`＝初回限定クーポン、`repeat`＝メニューが回数券・チケット消化）。
  **経路の分母は `first` のみ**にし、それ以外は `surveyFromNonFirst` として分けて出す
- CLI出力に「【1】はLINE・看板からHPBで予約した人も吸収する。広告の効果は【2】【3】の増減で見る」を常時表示
- 実物のメールで確認：初回客の回答0件・既存客1件として正しく分離された
- `docs/metrics-definitions.md` に制約2点（①既存客を分母に入れない ②【1】は他経路を吸収する）を明記。
  **LINE経由を確実に分けたいなら、アンケートではなくLINEのバナー先に別クーポンを割り当てる**（通知メールの
  「ご利用クーポン」で判別できる）ことも併記
- テストは65件すべてPASS（既存客を分母から外すテスト3件を追加）

## 2026-09-01 (Claude Code) 司令塔ダッシュボードが古い広告上限で判定していた（誤報の訂正と根治）
- オーナーの「広告の設定を変えたけど司令塔ダッシュボードってそれを認識してる？」から発覚
- 🔴 **認識していなかった。** ダッシュボードは **Meta ¥3,000／総額 ¥11,000** を基準に
  「Meta 日¥7,442（上限の2.48倍）」「総額 日¥12,428 で13%超過・3週目」と表示し、
  **8/25にオーナー承認で決着済みの「広告の日次上限を引き上げるか、支出を戻すか決める」を
  今週の宿題として出し続けていた**
- 現行値（**Meta ¥7,200／Google ¥4,800＝計¥12,000**・2026-08-25 の振替）で測り直すと
  **+3.4%／+3.6%** で誤差の範囲。総額は8/06から不変で、動いていたのは媒体間の配分だけ
- 🔴 **根本原因：司令塔のデータ源に `docs/ops-change-log.md` が入っていなかった。**
  「広告・シフト・サイト・掲載媒体に手を入れたら必ず1行足す」と決めたファイルを、司令塔だけ見ていなかった
- 対応3件：
  1. `docs/ads-ops-guardrails.md` を現行値へ更新（8/06のまま Meta ¥6,000 だった）。
     過去の上限の表と「**日次の消化額で超過と判定しない**」注意（MetaのCBOは日次¥12,600まで、
     Googleは予算の2倍まで振れる。判定は週ペース）を追加
  2. `andsync-command` スキルのデータ源に `ops-change-log.md` と `ads-ops-guardrails.md` を追加し、
     起動手順に「前回更新以降の変更ログを読む」を組み込み。焼き込まれていた前提値も訂正
     ⚠️ **同スキルは claude.ai 側の同期スキル。** ローカルの写しは直したが、
     **次の同期で戻る可能性があるため、多和田さんの環境での反映が必要**
  3. ダッシュボードを同URLへ再公開（14箇所訂正）。固定費カードの広告費上限（日¥11,000→¥12,000）と
     合計 ¥1,904,958→¥1,935,958・構成比も引き直した。「死角」に本件を解決済みとして記録
- **9/1 の広告設定変更は無かった**（オーナー確認済み）。`ops-change-log.md` の最新は 8/28 の新キャンペーン切替のままで、
  記録漏れではない。問い合わせの発端は「変更が反映されているか」の確認であって、未記録の変更ではなかった

## 2026-09-01 (Claude Code) 司令塔スキルを直接書き換えられない件の迂回 — Driveへ訂正ノートを投入
- オーナーの「きみが書き換えれないの？」を受けて、できること・できないことを切り分けた
- **ローカルの写しは書き換えられる（実施済み）が、それでは届かない。**
  `/root/.claude/skills/synced/` はセッション開始時に claude.ai から配られる写しで、コンテナは使い捨て。
  かつ**司令塔が動くのは別のチャット**で、そちらは claude.ai 側のスキルを読む。
  claude.ai のスキル本体へ書き込むツールは無い（読む・探すのみ）
- 🟢 **迂回路が有効だった。** `andsync-command` は起動時に必ず Google Drive `00_Inbox` を読み、
  **訂正ノートを最優先**で扱う仕様。そこへ訂正を置けば、スキル本体を触らずに次回起動から効く
- `00_Inbox`（`1OWupdmrjMcIK6DEqKRq_x3TFocQlVJwp`）へ訂正ノートを作成し、読み戻しで着信を確認：
  「【訂正】広告のガードレールは 日¥12,000（Meta ¥7,200／Google ¥4,800）— 司令塔の前提値が古い（2026-09-01）」
  （`1PJMaF6asuJ5JN_ijYCNMeCDE-fybePhdCBYaYcLjEyg`）
  内容：現行の上限／誤報の中身と正しい数字／日次で超過判定しない理由／上限の正本の場所／対応済みと未対応
- **これでスキル本体の編集は「やったほうがいい」だが「急がない」に落ちた。**
  ノートは次回1回だけでなく毎回読まれるので、編集しなくても誤報は止まる
- 学び：**焼き込まれた前提値を持つスキルは、書き換えられない場所にあることがある。**
  その場合は「スキルが必ず読む場所」に訂正を置くほうが速くて確実

## 2026-09-01 (Claude Code) 司令塔スキルの差し替え完了。訂正ノートの「未対応」を打ち消した
- オーナーが claude.ai の `andsync-command` を `docs/skills/andsync-command.SKILL.md` の内容へ**まるごと差し替え済み**
- 🟠 **後始末が1つ必要だった。** 直前に投入した訂正ノートに「**未対応（オーナー作業）：スキルを差し替える**」と
  書いてあり、そのままだと司令塔がそれを読んで**完了済みの作業を宿題に復活させる**。
  今日直したばかりの失敗（決着済みの論点を出し続ける）と同じ構造
- Driveコネクタの `update_file` はタイトルと置き場所しか変えられず本文を編集できないため、
  司令塔の運用ルール（**訂正は新規ノート・日付が新しいものが正**）に沿って完了ノートを新規作成：
  「【完了】司令塔スキルの差し替え済み — 広告ガードレールの訂正は対応完了（2026-09-01）」
  （`15se9fElWHg12Q7gnoiYTzQ6Grqij-9Aodk36ugoQf-U`）。
  前ノートの「未対応」を読んで宿題を復活させないよう明示し、有効な内容は引き継いで再掲した
- `docs/skills/andsync-command.SKILL.md` のヘッダと `docs/ops-change-log.md` にも完了を記録
- **検証日 2026-09-08**：司令塔が「予算超過」を出していないこと、変更ログを読んだうえで差分を報告していることを確認する

## 2026-09-01 (Claude Code) 週次売上の自動取得が払い戻しを引いていない件を検知し、恒久ルール化
- オーナーが店舗システムの画面値を提示 → 司令塔ダッシュボードの分析と突き合わせて検証
- 🔴 **ダッシュボードの数字が間違っていた。** 原因は `kpi-data:weekly_kpi.json` が**払い戻しを引いていない**こと
  ...
  差は **¥66,000 ちょうど**。8/30 16:43 の払い戻し1件（回数券とみられる）が未反映。
  来店38・新規22・更新2/3・指名11 は一致しており、**ずれるのは金額と販売本数だけ**
- 🔴 **1か所のずれが結論を変えていた。** 週商→4週平均→月換算→分岐点までの距離まで連鎖し、
  「①営業利益ゼロまであと13.5万＝週0.6本」が**実際は −20.7万**（月換算 ¥1,931,355 → ¥1,859,329）。
  成約率も 18.2%(4/22) → **13.6%(3/22)**
- 🟢 **死角がひとつ解消。** 「週商の内訳が合わない」は、実数だと
  施術¥113,200＋回数券¥189,640＋指名¥29,700＝¥332,540 でぴったり合う。
  合わなかったのは回数券売上を「本数×推定単価」で出していたため
- ⚠️ **`zenryoku-facts` の「回数券 平均単価 約52,000〜54,000円」は要見直し。**
  8/24週の実数は ¥189,640÷5本＝**¥37,928/本**。月商の計算根拠もこの前提に乗っている
- **結論の方向は変わらない**：回数券本数×週商 r=+0.98／新規来店×週商 r=−0.59（引き直しても同じ）
- **払い戻しは例外事象なのでスクレイパーでは自動控除しない**（オーナー判断）。
  代わりに「使う側が毎回検算する」形で恒久ルール化：
  - `docs/metrics-definitions.md`：「店舗システムの売上」の節を追加（正本）
  - `zenryoku-data` 4章に検算手順、**5章の優先順位を「オーナーが画面から読んだ値 ＞ KPIシート ＞ 自動取得」へ変更**
    （従来は自動取得が1位だった）
  - `zenryoku-facts` に 8/24週の確定値と平均単価の要見直しを追記
  - `zenryoku-biweekly-review` の「絶対に守ること」に検算を追加
  - Drive `00_Inbox` へ訂正ノート（`1dCo3_RXt_wSWIRwduxRGvV_VEKQU7in5MSywjZGZ7VA`）。
    リポジトリのスキルを読めないチャット（司令塔など）にも届くようにするため
- ⚠️ **未対応：司令塔ダッシュボードは8/24週を週商39.9万・成約18.2%・回数券6本のまま表示している**

## 2026-09-01 (Claude Code) 公庫Routineを有効化。ダッシュボードは自動更新に任せて仕組みを検証する
- **Routine「ANDSYNC 公庫返済スタート・資金繰り見直し」を有効化**（`trig_01PbB8i9moYrjL8MGJ7KVX98`）。
  読み戻しで `enabled: true` / `next_run_at: 2027-01-05T00:00:00Z`（JST 1/5 09:00）を確認。
  プロンプトの中身も確認済みで、元金返済の増加・役員借入金返済の見直し・2店舗目の資金余力の3点を
  Driveの該当ノート2本と突き合わせる作りになっている
  - ⚠️ モデルが `claude-opus-4-8` 固定。1年半先の発火なので、その頃には古い可能性がある。発火前に見直す
- **司令塔ダッシュボードの8/24週の誤りは、手で直さずに翌朝の自動更新に任せる**（オーナーが案Bを選択）
  - 理由：ダッシュボードは毎朝7:00 JSTに司令塔Routineが作り直す。手で直すと翌朝上書きされるため、
    **今日入れた訂正ノート経路が本当に効くかの検証にならない**。半日の誤表示より、検証できないまま
    仕組みを信用するほうが高くつく
  - **合格条件：9/2 朝の自動更新後、8/24週が 週商¥332,540・成約13.6%・回数券5本 になっていること**
  - 不合格なら訂正ノート経路が効いていないので、別経路を用意する
- `docs/ops-change-log.md` に上記2件を記録（検証日 2026-09-02 朝／2027-01-05）

## 2026-09-01 (Claude Code) GBP「検索面ゼロ」は実機確認で否定。口コミ返信率100%
- 🟢 **ダッシュボードの最重要課題「Googleビジネスプロフィールの露出が消えている」は誤り。**
  オーナーがスマホで検索したところ**シークレットモード（未ログイン）でも正常に表示された**。
  登録名も正（`全力ストレッチ岐阜長良店`）・審査待ちなし・`has_voice_of_merchant` true・重複なし。
  **プロフィールは出ている。ゼロなのは指標のほう**
- 🔴 **私はこの件で見立てを2回変えた。**
  ①「GA4のorganicが落ちていないから計測断の疑い」→ ②「8/22・23が埋まったのに8/24以降が埋まらないので実害の疑い」
  → ③「シークレットで表示。指標側で確定」。**数字だけで往復させた。実物を1回見れば済んだ**
- 判断ルールを `docs/metrics-definitions.md` に追加：
  - `*_search` が0でも `impressions_mobile_maps` と `direction_requests` が動いていれば**まず指標の故障を疑う**
    （本当に消えたならマップ面も一緒に落ちる）
  - **データ側の議論で結論を出さない。** 実機・未ログインでの検索が唯一の確定方法
  - GBPのレポートは後から埋まる（8/22・23のゼロは数日後に70・45へ）。**直近2〜3日のゼロを異常として報告しない**
- 「名前が『全力ストレッチ店』へ戻る」件は直接の原因ではなかった（現在は正しい名前・審査待ちゼロ）。
  ただし検索キーワード実績に「住所＋全力ストレッチ店」形のクエリが複数あり、**外部データに
  その表記が存在する可能性**は残る。オーナーが「全力ストレッチ店」で検索して出所を探す（未着手）
- 🟠 **口コミの総数を取り違えていた。** APIの既定取得窓が直近30日のため「18件中18件＝100%」と報告したが、
  **総数は80件**。`date_preset: last_2years` で全件取得したところ、過去分はスタッフが丁寧に返信済み
  （1★・2★にも対応あり）で、溜まっていたのは直近だけだった
- 8/29の新着2件にも返信し、**80件中80件＝返信率100%**（平均4.7）
- `docs/gbp-ops.md` に返信のトーン指針を追加：**2行程度・堅くしない・余計なことを書かない**、
  **「溜まってきたら」は使わない**（お客様の状態を否定的に決めつけ、再来店を「不調になったら来る」ものとして描くため。
  メンテナンスとして通ってもらう設計と矛盾する／オーナー指示）、**スタッフ名を返信で繰り返さない**

## 2026-09-01 (Claude Code) 名前の巻き戻りの仮説は外れ。追うのをやめて検知に切り替えた
- 私が立てた「外部データに `全力ストレッチ店` の表記があり、Googleがそちらへ寄せている」という仮説を
  オーナーが `全力ストレッチ店 岐阜` で検索して検証 → **そのような表記を使っているサイトは無かった。仮説は外れ**
- 現況は名前が正・審査待ちなし・`has_voice_of_merchant` true・重複なしで**実害なし**。
  **実害の出ていないものを推測で追いかけない**と決めた（今日繰り返した失敗と同じ構造のため）
- 代わりに**検知に切り替え**：`fetch-windsor-data.mjs` に `gmb_profile_state` を追加し、
  `location_title` / `has_pending_edits` / `has_google_updated` / `has_voice_of_merchant` /
  `duplicate_location` / `review_total_count` / `review_average_rating_total` を日次で記録。
  次のいずれかで異常とする：名前が正でない／審査待ちが立つ／Voice of Merchant が false ／重複あり
  - 再発したら**発生時刻と前後の出来事が残る**ので、そこから原因を追える
- あわせて `gmb_reviews` の取得窓が**直近28日ぶんしかない**ことをスクリプトのコメントと
  `zenryoku-data` に明記（今日「18件中18件＝100%」と報告して総数80件を取り違えた再発防止）。
  返信率を言うときは `gmb_profile_state.review_total_count` を分母にする

## 2026-09-01 (Claude Code) Google広告サイトリンク4本／GBPのサービス・説明文を更新
- **スケジュールタスクの削除3件は不要だった。** 削除を試みたところ「not found」。再取得すると18件→14件で、
  対象3件（コラム自動生成／週次マーケレポート／スガワラくん動画）は**すでに存在しなかった**。
  ⚠️ 私が「残す」と勧めた**第二店舗 物件ウォッチも消えている**。私の操作ではない
- **Google広告にサイトリンク4本を追加**（campaign `24029290016`）。料金・メニュー／アクセス・駐車場／
  お客様の声／お悩み別のご案内。薬機法チェック exit 0・文字数制限内で確認してから実行。
  **Windsorに `create_ad_asset` があり、sitelink/callout/structured_snippet/call を作れる**（今回初めて使った）
  - ⚠️ 新規サイトリンクは**Googleの審査を通ってから表示される**
- **GBPのサービスを3コースで登録**（旧「ストレッチ」1件・価格なしを置き換え）。
  通常価格を `price`、初回価格を説明文に併記（初回だけ出すと二重価格表示になるため）
  - ⚠️ `price` は**男性料金に揃えた**。2回目以降は性別で異なるがGBPは1サービス1価格しか持てない
  - ⚠️ `update_service_items` は**全置換**。次に変えるときは既存分も送り直す
- **GBPの説明文を差し替え**（457字・薬機法チェック exit 0）。旧文は「即効」で落ちていた。
  「全員女性のトレーナー」は事実確認が取れていないため入れていない（確認できれば追記可能）
- 🔴 **予約リンク属性は設定できず、しかも私の見立てが誤りだった。**
  `url_appointment` はプレフィクス有無の両方で Google が400。検索結果のスクショに
  **「予約：hotpepper.jp プロバイダ」**とあり、**予約導線はHPBの予約プロバイダ連携ですでに入っている**。
  連携済みの場合Googleは手動の予約URLを受け付けない。**8/31の「予約リンク未設定」という指摘は取り下げる**
- ⚠️ **写真の追加はできていない。** 素材（外観・内観・受付・個室・駐車場・ロゴ）が無いため。
  公開URLに置いていただければ `upload_media` でカテゴリ指定つきで投入できる
- ⚠️ **完全個室・女性スタッフ等の属性は未設定のまま。**
  Windsorから利用可能な属性IDを列挙する手段が無く、当てずっぽうで叩くと400が返るだけなので、
  **選択肢が一覧表示されるGBPの管理画面で設定するほうが確実**

## 2026-09-01 (Claude Code) GBP写真は8枚すべて使う（オーナー判断で私の推奨を変更）
- 素材50枚超のうち `photos-src/` の8枚を目視で確認し、2群に分類
  - A：施術が主題（pic-9 / 33 / 66 / 78）→ `gbp-treatment-01〜04.jpg`
  - B：トレーナーの姿が主題（pic-15 / 37 / 49 / 58）→ `gbp-trainer-01〜04.jpg`
- 私はBを「使わない」と推奨したが、**オーナー判断でBも使う**ことになった
  > 「健康志向のあまりないこういう客にリーチできるのも全力ストレッチの強み」
  - **店の位置づけの決定。** 健康・コンディショニング層だけでなく、そこに入ってこない層まで取りにいく戦略。
    `gbp-ops.md` に「今後この判断を蒸し返さないこと」として明記した
- 🔴 **自分の論拠のひとつが弱かったので訂正して残した。**
  「本部バナー（被写体が全員女性）が234クリックでLead 0件」を根拠に挙げたが、
  これは**停止時に私が挙げた第一候補の仮説であって検証されていない**。
  クリエイティブ・文面・価格訴求も同時に違う。**根拠として持ち出さない**と `gbp-ops.md` に書いた
- 8枚ともGBP規格に変換済み（長辺1600px・JPG・100〜240KB）
- 見直しの条件も決めた：**新規の質（キャンセル率・回数券成約率）が落ちたら構成を見直す。**
  印象論ではなく数字で判断する
- ⚠️ **投入は本番デプロイ後。** `upload_media` は公開URLしか受け付けず、
  `zn-stretch-gifu.com/images/gbp/<名前>` が生きるのはデフォルトブランチへ取り込んでから
...
## 2026-09-06（続き）— 店舗システムから稼働・売上内訳を自動取得、勤務時間は取得できず

### 背景
「シフトシステムを見れば実際の給与はわかる」との指摘を受け、kpi-scraperの
探索モードで店舗システムのページ構造を調べ、取得対象を広げた。

### 分かったこと
- **店舗システムに給与額は存在しない。** 勤務時間ページの列は
  `年月 / 担当 / 勤務時間 / 勤務日数 / 遅刻・欠勤 / 備考` のみ。時給・総支給・
  バックの列はない。取れても基本給（実働×1,600円）までで、指名バック・
  回数券バック・手当は9/15の給与明細を待つ必要がある。
- **勤務時間ページは2026-08で行が0件。** ナビは開き月の入力も適用されたが、
  テーブルは7列に対して1行1セル＝空状態のプレースホルダ。
  **この店舗システムのこのログインでは、8月の勤務時間レコードが返らない。**
  原因（月フィルタがサーバ側に効いていない／アカウントの店舗スコープ／
  そもそも未入力）は画面を見ないと切り分けられない。オーナー確認が必要。
- **稼働は月次で取得できた。** 分母はベッド4台×日数×12h。
  6月12.9%(185.5h) / 7月19.4%(289h) / **8月18.8%(280h)**。
  旧記載「8月13.5%」は月途中の値だった。
- **月次の新規来店：7月119人 → 8月87人（−27%）。** 同期間の広告費は
  経常ベースで約¥295,000→¥379,527（+29%）。新規1人あたり約¥2,479→約¥4,363で
  **1.76倍に悪化**している。
- **売上の内訳が自動で取れるようになった。** 施術113,200＋回数券189,640＋
  指名29,700＝332,540 が sales_total と一致。2026-09-01の恒久ルールの検算が
  毎週手作業なしで回る。sales_total と new_ticket_sales も払い戻し反映後の
  値（332,540／3本）で返り、オーナーの確定値と一致した。

### 変更したファイル
- `kpi-scraper/fetch.mjs` … navClick（サイドバーをtextContent完全一致でクリック）、
  doFillsに{month}プレースホルダとinput/changeの明示発火、table_rowsの
  「行なし」エラーに構造情報（行数・列数）を追加
- `kpi-scraper/selectors.json` … 勤務時間ページ、売上内訳3項目、稼働（月次）を追加
- `.github/workflows/kpi-data.yml` … 今回生成しなかったファイルを既存ブランチから
  引き継ぐステップを追加（探索モードで weekly_kpi.json が消える問題の修正）
- `.claude/skills/zenryoku-facts/SKILL.md` … 稼働率の確定値、月次の来店・新規を反映

### 未対応
- 勤務時間が0件になる原因の切り分け（オーナーが画面を確認する必要あり）
- 8月分給与（9/15支給）の確定額
- 「稼働率」の定義が3つ流通している（ベッド時間1,488h基準／施術時間推定／
  勤務時間に対する埋まり方）。1か所に固定する

## 2026-09-06（確定）— 8月の人件費を Shift-management の Supabase から実額算出

### 背景
オーナーの指摘「リポジトリのShift-managementで給与額は見れる」を受けて、
`andsync-y/Shift-management`（Supabase project `shift-kanri`）の給与モジュールから
実額を計算した。店舗システム（system.zn-stretch.com）には勤務時間しかなく、
給与額はこちらが正本だった。

### 検証
`src/lib/payroll.ts` のルール（休憩自動控除・1分単位・残業1.25倍・深夜+0.25・
交通費15円/km往復）で `time_records` の実打刻から7月を再計算したところ、
**¥1,215,862** となり、給与明細集計の実績 ¥1,214,699 と **0.1%差で一致**した。
モデルは正しい。

### 分かったこと
- **8月の総支給は ¥1,080,724。** 基本給¥854,077（実働533.8h×¥1,600）＋残業¥1,623
  ＋深夜¥904＋交通費¥30,120＋**指名バック¥150,000（50件）**＋**回数券バック¥44,000（24本）**。
- **【解消】回数券バック0本は計上漏れではなかった。** 2026-08-23に「7名全員0本」として
  疑ったが、月末時点では24本・¥44,000が正しく入っていた。8/23は未入力だっただけ。
- **推計が¥150,724低かった主因は指名バック。** 8/22時点で8件だったものが月末50件になり、
  ¥24,000→¥150,000と6倍になっていた。
- **指名は素通りしている。** バック¥3,000/件は「指名料の税抜相当・税引後100%還元」。
  8月は50件で売上約¥165,000に対しバック¥150,000＝店の取り分は¥15,000（9%）。
  指名が増えても利益はほぼ増えない。指名率をKPIに置く意味も薄く、方針確認が要る。
- **損益分岐点① は ¥2,368,602。** 人件費¥1,215,233（総支給＋社保¥121,000＋労働保険¥13,509）
  ＋その他販管費¥968,618 ＝ 固定費¥2,183,851 ÷ 0.922。
  同日午前の推計¥2,245,383より **+¥123,219**、当初の¥2,190,475より **+¥178,127**。
- 現在地（直近4週平均の月換算¥1,859,329）との差は **−¥509,273 ＝ 回数券 月13.4本**。
  午前に「月+8.7本」と書いたが、実額では13.4本。8月実績24本なので月37本が必要。
- 広告削減案（月−¥152,347）を入れると分岐点は¥2,203,367、差は月9.1本。**削減の価値は上がった。**

### 未解決
- 店舗システムの画面が示す「先月の稼働率 9.9%」と、スクレイパーが取った8月18.8%が合わない。
  147（8月来店数）÷1,488＝9.88%と一致するため、画面は1来店を1時間として数えている
  可能性がある（未確定・オーナー確認1問）。**どちらでも「設備は制約でない」結論は変わらない。**
- 「稼働率」の定義が3つ流通している。`docs/metrics-definitions.md` に1本化する。

### 変更したファイル
- `.claude/skills/zenryoku-facts/SKILL.md` … 分岐点①を実額¥2,368,602へ、人件費の内訳、
  人件費のデータ源（Supabaseが正本・freeeは1か月ズレるので使わない）、指名の素通り、
  回数券バックの段階単価、稼働率の食い違いを追記

### 成果物
- Drive 00_Inbox `2026-09-06_【確定】8月の人件費と損益分岐点_Shift-managementのSupabaseから実額算出.md`

## 2026-09-07 — 広告費の判断を訂正：Metaは切らない。8月の新規減は「6月に戻った」

### 背景
オーナーから「新規−27%は広告を変えたせいでは。8月に増額を勧めたのに下げてよいのか」との指摘。
デフォルトブランチをマージして初めて読めた `docs/channel-analysis-2026-08.md` と
`docs/ads/strategy-options-2026-08-28.md` で検証した結果、指摘が正しかった。

### 分かったこと
- 再来店を除き31日換算すると新規は 6月73→7月113→8月70。**8月は6月水準で、7月が特異点。**
  「7月→8月で−27%」は2か月比較の誤読だった。
- 落ちたのはインスタ広告経由だけ（24.8→29.0→10.8）。原因は8/1のクリエイティブ全面差し替えで
  価格明示型が消えたこと。**広告を変えたから落ちた、が正しい。**
- 8/22の「LPビュー単価不変＝問題なし」も、9/6の「予約クリック単価¥503＝好調」も、
  同じ中間指標の罠。来店経路別と突き合わせていなかった。
- 立て直し（案A：初上陸バナーD＋価格明示、¥7,200全額、判定9/8〜9/12）はすでに実行中。
  直近7日はDが¥40,298（80%）、Bは¥515で実質停止。
- **9/1〜6の経路別新規：インスタ広告5人（動画4＋初上陸1）＝週5.8人ペース＝月約25人。
  6月水準（24.8）に戻り始めている。** HPBは6人＝月31ペースで相変わらず安定。
  受付の「きっかけ」に「インスタ広告（初上陸）」が追加されており、Dの帰属が取れるようになった。

### 訂正した判断
- Meta：**9/12の判定まで¥7,200維持**。判定線は「インスタ経由の新規が週6人ペース」。
  割ったら¥3,500＋6月型クリエイティブで再試行。0にはしない。
- Google：¥4,800→¥3,000。経路別に痕跡がないが、Googleマップの伸びが検索広告開始と
  同期している可能性が残るため、3週見て戻す。
- HPB：維持（ベースロード）。
- 広告より大きいレバー：担当Aの成約率13%（新規の37%を受けている）。平均26.9%なら
  回数券+3.2本＝月約¥165,000。シフトの平日→土日再配分（−¥165,000〜215,000）とあわせて、
  広告に触らずに経常−63万の半分以上が埋まる。

### 変更したファイル
- `.claude/skills/zenryoku-facts/SKILL.md` … 月次新規の読み方の訂正、インスタ広告経由の
  月次推移、広告判定に使う指標の限定、Metaの判定線を追記

### 成果物
- Drive 00_Inbox `2026-09-07_【意思決定】広告費と人件費をどうするか` を本内容で書き換え

### 未対応
- 9/12の判定（インスタ経由の新規が週6人ペースを維持しているか）
- Googleの3週観察（Googleマップ・HP経由が落ちないか）
- 担当Aの成約率是正（`closing-coach`）、シフト再配分は別チャットの担当

## 2026-09-07 — Google検索キャンペーンを停止（オーナー承認・実行済み）

### 実行
- Windsor `execute_action(google_ads, 600-036-3311, pause_campaign, {campaign_id: 24029290016})` → 成功。
  復旧は `enable_campaign`。

### 理由
「¥3,000へ下げる」より「止める」を選んだ。検索広告→店名認知→Googleマップ来店という間接効果の
仮説（マップ経由が広告開始後に月3.1→11.0→9.4人）は、下げても検証できず、止めれば3週で分かる。
Metaの判定は経路行「インスタ広告」だけで行うので、Google停止に汚されない。

### 判定線（9/28頃）
Googleマップ＋HP経由の新規が月6人未満 → 再開（日¥3,000・指名語中心）。維持なら停止のまま。
最悪の下振れは月−¥49,000で可逆。

### 変更したファイル
- `docs/ads-ops-guardrails.md` … 停止の記録・判定線・復旧手順
- `.claude/skills/zenryoku-facts/SKILL.md` … 現在の配分を「Google停止中」へ

## 2026-09-07（訂正3）— 6月は11日営業。開店以来の単調減少で、9月に反転

### 背景
オーナー確認：6/8研修開始・6/15プレオープン・**6/20予約受付開始**。
`docs/channel-analysis-2026-08.md` は6月を30日営業として31日換算しており、
**6月の実力を2.7分の1に潰していた。**

### 営業日あたりに直すと結論が反転する（新規/営業日・再来店除く）

| 経路 | 6月(11日) | 7月(31日) | 8月(23日) | 9月(6日) |
|---|---|---|---|---|
| HPB | 2.73 | 0.94 | 1.00 | 1.00 |
| インスタ広告 | 2.18 | 0.94 | 0.35 | 0.83 |
| 合計 | 6.45 | 3.65 | 2.26 | 3.00 |

- 🔴 取り消し：「8月は6月に戻っただけ。突出は7月」→ 正しくは**開店以来の単調減少で9月に反転**。
- 🔴 取り消し：「HPBは3ヶ月31/29/31で異常に安定」→ 6月は2.73/日で7月以降の2.9倍。
  安定しているのは7月以降。
- ✅ 生存：「インスタ広告の落ち込みは8/1の差し替えが原因」→ 6月を使わず7月→8月で
  0.94→0.35（−63%）。同じ結論。**9月は0.83で7月水準の88%まで回復**（初上陸バナーD）。
- ✅ 生存：「GoogleがHPB経由を動かしていない」→ 7月→8月でGoogle支出4.1倍に対しHPBは+6%。
  6月を使わずに言えるので開店特需の影響を受けない。

### もう一つの発見：予約導線の歩留まりが10%
30日で WEB予約(HPB)クリック537／LINE 52(9%)／電話15 に対し、オンライン経由の新規は約54人。
**クリックの90%が予約に至っていない。** HPB側のファネルは未計測（会員登録の壁の疑い）。
歩留まり1.5倍で新規+27人/月＝**+¥330,407**で、これは広告費全額（¥332,110）に相当する。
**広告予算の調整より大きいレバー。**

### 変更したファイル
- `docs/channel-analysis-2026-08.md` … 【訂正3】を追記（本文は残し、旧表を無効と明示）
- `.claude/skills/zenryoku-facts/SKILL.md` … 開店の経緯、新規/営業日、比較の鉄則、
  予約導線の歩留まりを追加

### 判断への影響
Meta維持（Dが7月水準の88%まで戻している）とGoogle停止（HPBが4.1倍の支出に無反応）は
いずれも変わらない。ただし「Google広告は効かない」ではなく
**「漏れているファネルの上では効果を判定できない」**が正しい表現。

### 次にやること
- HPBの集客プラン管理画面でアクセス数→予約数のファネルを取る（担当営業に場所を聞く）
- HPBの「会員登録なし予約（ゲスト予約）」が有効か確認
- LINE予約の格上げ（現状9%）

## 2026-09-07（続き）— HPBのファネル実数を取得。診断は「集客は強い、予約で負けている」

### 取得したもの
オーナーがHPB集客プラン管理画面のCSV（日次・月次）を提供。8月実績：

| | 自店 | エリア平均 | 比 |
|---|---|---|---|
| 総PV | 10,345 | 5,855 | 1.77倍 |
| クーポンメニューPV | 1,864 | 1,069 | 1.74倍 |
| CVR | 53.8% | 48.3% | 1.11倍 |
| **ACR** | **4.8%** | **5.9%** | **0.81倍** |
| 予約数 | 90 | 67.5 | 1.33倍 |
| 予約売上高 | ¥363,400 | ¥400,839 | 0.91倍 |

### 診断
**集客は強く、予約への転換で負けている。** ACRがエリア平均なら月110件で **+20件/月**。
最有力の原因は**土日の空き枠不足**で、`docs/capacity-analysis.md`（土日充足率65〜79%・出勤3.0名）と
整合する。**シフトの問題と予約の問題は同じ問題だった。**

また、LPが送る537クリックは自店のPV→予約率2.22%で換算して月13件程度で、
**HPB予約90件の85%はHPB自身の集客**。HPB掲載料¥45,100は引き続き最良のチャネル。

### 訂正
- 🔴 「TOPページにスティッキーCTAが無い」は誤り。`src/layouts/Base.astro` に全ページ共通で存在する。
- 🔴 「LINEは会員登録なし」も誤り。友だち追加は必要。さらに**当店のLINEは予約導線が無く**
  （友だち追加→氏名→登録完了で終わり）、LPの「LINEで予約」は行き止まりになっている。
  導線を直す前に宣伝しようとしていた。

### 分かったこと（FC内の先行事例）
富山店がLINE直予約を導入済み（`salons.jp` のLIFF・マルチテナントSaaS）。
**本部一括ではなく個別契約**のため、岐阜長良に近道はない。
損益中立は 月額÷¥11,303人/月（例：¥20,000→1.8人）で、ハードルは低い。
論点は費用より「LINEで予約したい人がいるか」。

### 次にやること（優先順）
1. **土日シフト4名**（費用ゼロ・ACR改善で+20予約/月・在庫を作る）
2. LINE公式アカウントのあいさつ文とリッチメニューを無料で直し、相談→手動確定で3〜4週テスト
3. その結果を見て salons.jp の契約を判断（HPBとの在庫同期の可否が前提条件）
4. LP専用クーポン（LP由来の予約13件（推定）を実数にする）
5. GBP投稿にBOOK CTA

※ 富山店の予約URLに含まれる session_token は認証情報のため記録していない。

## 2026-09-11 — Google広告クレジット¥67,000の扱いを決定（いま再開しない）

### きっかけ
オーナーがGoogleから「¥67,000分の広告クレジット獲得のお知らせ」を受け取り、
「使った方がいいよね？」と相談。

### 判断：使う。ただし9/28の判定を終えてから

いま再開すると 9/7 から走っている3週の自然実験（Google検索を止めて
マップ＋HP経由の来店が落ちるかを見る）が壊れる。実験で「効いていない」と確定すれば
**月¥167,451を恒久的に止められる**ので、情報の価値がクレジット¥67,000を上回る。
Google広告のクレジットは通常60日程度の期限があり、9/29再開でも間に合う見込み。

### 有効期限が確定（同日中にオーナーが画面で確認）

**広告配信の費用の有効期限＝2026-11-08。** プロモーションコード `CW9LN-WKQRR-GWC9`、
ステータス「有効」、使用済み¥0（手つかず）。

画面の3つの日付を取り違えないこと:

| 画面の項目 | 日付 | 意味 |
|---|---|---|
| 適用日 | 2026-07-14 | プロモーションが付いた日 |
| 要件達成期限 | 2026-09-12 | 掲載条件（¥67,000分）の締切。**達成済み**なので以後無関係 |
| **広告配信の費用の有効期限** | **2026-11-08** | **クレジットを使い切る締切。実質の期限** |

9/29再開なら11/8まで40日あり、日¥3,000（22日で消化）でも10/21ごろに完了。18日の余裕。
**待つ判断で問題ないことが確定した。**

### 見落としやすい点（記録した理由）
¥67,000は**日¥6,000なら11日で尽き、その後は自動で現金請求に戻る。通知は来ない。**
再開時は日¥3,000（22日分）とし、消化見込み日にリマインダーを置くこと。

### 新しく増えた期限
判定が「再開しない」に出た場合でも、**11/8までに使い道を決めないと¥67,000は消える。**
判定の結論とは独立に拾う必要があるため、11/1のリマインダー
（`trig_01Cck3pxw8zzybzL621sBK74`）を別途設定した。

### 変更したファイル
- `docs/ads-ops-guardrails.md` … 「2026-09-11 Google広告クレジット ¥67,000」節を追記
- 9/28判定のRoutine（`trig_011s1n1bAjEfo66ibb4qU9G3`）のプロンプトに同内容を追記
- 11/1の失効前リマインダー（`trig_01Cck3pxw8zzybzL621sBK74`）を新規作成

### 本日の定期タスク
Instagram投稿ドラフト（テーマ：反り腰の壁チェック／記事slug `sorikoshi-check`）を
Drive 00_Inbox へ保存。薬機法チェックは `scripts/yakkihou-ng.mjs` で本文・ハッシュタグとも
NG 0件。リポジトリへの変更は無し。

⚠️ **Instagram試用の判定が期限切れ。** 「2週間で4本投稿できたら継続」で8/25に開始したため
9/8で試用終了。下書きは6本生成済みだが、**実際の投稿本数がオーナーから未報告**。
継続/停止の判断待ち。

## 2026-09-12 — 回数券販売率の低下は広告のせいか（オーナーの問いを検証）

### きっかけ
「広告を変えて新規が増えた気がするが、回数券の販売率が落ちている。女性比率も上がっている。
販売率が下がった要因が広告の可能性はないか？」

### 結論
**可能性はある。ただし現時点では「落ちた」こと自体が統計的に確認できていない。**

- 直近2週 新規42/券6＝**14.3%**、変更前6週 **19.7%**。P(≦6/42|p=25%)＝**7.1%** で有意ではない
- 週の新規は約20人。1SD＝9.7ptで、真値25%でも**6%〜44%**で揺れる。7/20週10.7%・8/10週47.1%・
  8/17週5.3%はいずれも施策と無関係の振れ
- 9月の経路別では **Meta広告経由 6人/1本＝16.7%（全体平均と同じ）／HPB 6人/0本＝0%**。
  **落ち込みを作っているのは広告ではなくHPB経由**（ただしn=6で、これも誤差の範囲）

### 見つけた構造（オーナー確認済み・確定）
🔴 **新規向けの初回特別価格（最大20%OFF）は男性コースのみ。女性には初回割引を出していない。**

- 男性・90分10回券 ¥154,000 → **¥132,000**（その場で¥22,000引き＝「今日買う理由」がある）
- 女性・90分10回券 ¥95,700（**初回割引なし**）。しかも都度払いが元から安く乗り換え動機も弱い

**「女性は回数券を買わない」のではなく「女性向けに売る設計になっていない」。**
加えて女性の券単価は男性の初回価格の**72〜80%**で、女性比率10%→30%なら
成約率が同じでも平均単価が約5%（月約6万円）下がり、損益分岐点が上振れする。

### 自己点検（2件）
🔴 **女性比率が上がった原因は 2026-08-28 の Advantage+ 有効化。意図的にそうした。**
判断根拠は「7月実測で女性のLead単価¥28<男性¥29」＝**中間指標**で、来店・成約では検証していない。
`zenryoku-facts` の「広告の判定は店舗システムの経路別新規のみ」に反しており、
8/22・9/6 に続く**3度目の同じ誤り**。

🔴 **8/28は5変数を同時に変えた**（新キャンペーン／最適化対象LPV→Lead／年齢下限35→25／
Advantage+オン／クリエイティブD＋CBO）。**原因の切り分けが原理的に不能。実験として失敗。**
以後、広告の変更は**1回に1変数**とする。

### 新たに発見した指標の問題
**「新規販売率」に水準の違う2系列が併存していた。** ①店舗システムの週次そのまま（6週で19.7%）と
②再来店を除いた月次（7月24.4%・8月26.5%）。5〜8ポイント違う。**同じ表に混ぜない。**

### 次の検証
オーナーが**来店記録をCSVで出せる**（確認済み）。性別×新規販売率を直接検定する。
⚠️ CSVには個人情報が含まれる。集計値のみを残し、氏名・電話番号・メールアドレスは書かない。

### 変更したファイル
- `docs/ticket-rate-analysis-2026-09.md`（新規）… 分析の本体。打ち手候補A〜Dと検証手順
- `.claude/skills/zenryoku-facts/SKILL.md` … 回数券オファーの性別非対称／女性の券単価／
  女性比率上昇の原因／単週で判断しない／2系列の併存／広告は1回1変数
- `docs/metrics-definitions.md` … 新規販売率を単週で判断しない、2系列の注意
- `docs/ops-change-log.md` … 8/28の変更を「実験として失敗」と判定
- 9/12 Meta判定Routine（`trig_016A7Lgrwpb5Kc32jV2Yj2jD`）に、率と本数の併記・
  「成約率の変化を広告のせいにしない」・CSVの催促を追記

### 確認したこと
薬機法チェックは新規作成分（`ticket-rate-analysis-2026-09.md`）で終了コード0。
`metrics-definitions.md` と `zenryoku-facts` に残るNG検出は**いずれも今回の追記以前からある行**で、
社外に出す文面ではないため手を入れていない。

## 2026-09-13 — 来店記録CSVで直接検証。前日の結論を全面的に入れ替えた

### きっかけ
オーナーが来店記録CSV（2026-06-20〜09-12・430件／新規304件）を提供。
あわせて「女性コースの初回割引はFC規約的に難しい」「FCの規約って把握してるよね？」の指摘。

### 撤回した主張（5件）

| 撤回 | 何が誤りだったか |
|---|---|
| 「女性比率の上昇が販売率低下の主因」 | 女性は新規の8.9%で寄与が小さい |
| 「女性比率の上昇はMeta広告のAdvantage+のせい」 | **Meta経由の女性比率7.5% < HPB 13.9%。** 女性27人中Meta経由は4人。上昇は8/1から始まっており8/28より前 |
| 「打ち手は女性コースの初回割引」 | **FC規約を未確認のまま価格改定を提案した。** 第11条2・3項で本部の事前許可が必須、違約金1,000万円 |
| 「主因は担当者」 | 担当者差は全期間では実在（p=0.0002）だが、**直近の落ち込みは7名中6名が同時**で、これでは説明できない。別の現象を混ぜていた |
| 「配分の入れ替えで月¥200,000」 | **指名の制約を入れていなかった。** 動かせるのは7枠、上限¥72,971／4週 |

### 分かったこと

**落ち込みの境目は 2026-08-17。** 〜8/16 19.2%（45/234）→ 8/17〜 10.0%（7/70）。
Fisher p=0.102 でまだ有意ではないが、**7名中6名が同時に低下**し、
最も成約率の高かったAINA（38.9%）が8/12を最後に6人みて0本。
**年代・経路・担当・性別のどれで標準化しても期待値（16.5〜21.8%）が実績を上回る＝
客層の変化では説明できない。8/17前後に何が変わったかは未解明。**

**オーナーの仮説「売れる人ほど新規が回らなくなる」はデータで裏付けられた。**
相関 +0.93（販売本数→後のチケ消化数）／−0.74（→後の新規比率）。
ただし落ち込みの**30%**しか説明しない。

**そしてこの循環は指名によって不可逆。** 指名ありの消化は本人しか対応できないため、
AINAの消化18枠のうち**17枠が固定、動かせるのは1枠**。上位3名でも7枠が上限。
消化に占める指名比率は 75%→66%→**78.9%** と上昇中で、**放置すると自動的に悪化する。**

**レバーの大きさ（4週）**：下位3名の育成 ¥181,627〜¥350,460 ／ AINAのシフト+1日 ¥118,763 ／
配分の入れ替え ¥72,971。**下位3名が8/17以降の新規70人中42人＝60%を担当している。育成一択。**

### FC契約を読んだ（初めて）
`docs/fc-rules.md` を新設。**価格・メニュー・給与/報酬形態・商標使用は本部の事前承認が必須**
（第11条2・3・5項、第13条2項／違約金1,000万円）。
**広告宣伝方法・営業時間・スタッフ数・シフトは加盟店の裁量**（第11条6項）。
ライセンス料 月¥110,000、ベッドは1店舗4台まで（5台以上は2店舗分の加盟金・ライセンス料）。

⚠️ **第11条8項（顧客リスト・FC内人脈への独自商品/サービスの営業禁止・違約金1,000万円）は、
以前話していた「予約システムをパッケージで販売」に正面から当たる可能性が高い。要確認。**

### データの検証
**CSVの8月＝回数券24本・¥1,238,930 で店舗システムの月次画面と完全一致。**
2026-08-23に疑った入力漏れではない。

### 再発防止
- 原因を語る前に**店舗システムの実データで直接検定する。** 中間指標と推論で筋を立てない
- **価格・メニュー・給与形態に触れる提案の前に `docs/fc-rules.md` を読む**
- 「主因は◯◯」と書く前に、**その説明で何%が説明できるかを計算する**

### 変更したファイル
- `docs/ticket-rate-analysis-2026-09.md` … 全面改訂（結論・撤回一覧・レバー・次にやること）
- `docs/fc-rules.md`（新規）… FC契約の要点。**契約書本体はリポジトリに置かない**
- `.claude/skills/zenryoku-facts/SKILL.md` … FC契約・ライセンス料・ベッド上限、
  性別/年代/担当×成約率の実数、8/17の境目、指名ラチェット、女性比率の原因の訂正
- Drive `40_FC規約・契約` フォルダを新設（オーナーが契約書とオーナーズマニュアルを格納）

### 未読
オーナーズマニュアル（PDF 31.5MB）は未読。価格・キャンペーンの運用規定が載っている可能性がある。

## 2026-09-14 — 経路別の分析を撤回。土日4名化も格下げ

### きっかけ
オーナーから「その経路って予約メールと店舗システムも確認してる？」と指摘。
確認したところ**店舗システムのCSV1本だけ**で、突き合わせていなかった。

### 撤回したもの
🔴 2026-09-13に出した**経路別の月次比較を全部撤回**。

- 「Meta広告だけが増えている（0.41→1.00で2.4倍）」
- 「犯人は広告ではなくHPB（−0.66/日）」
- 8月→9月の経路別の増減すべて

**理由：2026-08-31にHPBの経路アンケートを表示開始しており、計測仕様が期の途中で変わっている。**
未記入は 8月13.8% → 9月3.7%。**Meta +0.59/日 と 未記入 −0.47/日 がほぼ打ち消し合い、区別できない。**
さらに経路の選択肢自体が月ごとに入れ替わっていた。

`zenryoku-data` に自分で「計測の仕様が変わった日をまたぐ期間は前後の数字を同じ列に並べない」と
書いておきながら、それをやった。

### 残る事実
**新規/営業日は分類に依存しない。** 6.45 → 3.84 → 3.22 → **2.45**（9月は8月の76%・7月の64%）。
新規の数と成約率が同時に落ちている。

### 9月単月の女性の寄与を検定
9月 男性 3/23＝13.0% ／ 女性 0/4＝0%。女性比率は14.8%まで上昇しているが、
**寄与は0.8pt＝落ち込みの約11%**。**男性だけでも 8月22.4% → 9月13.0% で同じように落ちている。**

### 「土日4名化」を格下げ
オーナー確認：**土日は予約できる状態（枠は空いている）／土日に出られるスタッフがいない**。
根拠の自然実験はn=3日、充足率も土65%・日79%で満杯ではなかった。**枠不足という前提が崩れた。**
**平日（木・金）の供給過剰は事実のまま**なので、削減対象は木金水に限定する。

### 接続の問題（次回のために記録）
予約通知メールは `zngifunagara@gmail.com` に届くが、**Claudeに接続されているのは
`tawada.4510@gmail.com` と `y_tawada@bitsense.co.jp` のみ**。`salonboard.com` からのメールは0件。
**突き合わせにはこの口座の接続が必要。**

### 変更したファイル
- `docs/metrics-definitions.md` … 「来店経路の月次比較は使えない」を追加（理由3つと決着方法）
- `docs/capacity-analysis.md` … 「土日4名化は根拠不十分」の訂正章を追加
- `docs/ticket-rate-analysis-2026-09.md` … 経路軸の標準化に注記、9月単月の女性検定と土日の確認を追記
- `.claude/skills/zenryoku-facts/SKILL.md` … 上記5点

### 未対応（オーナー判断待ち）
- スタッフ名（AINA等）と個人別成約率がリポジトリに残っている。記号化してDriveに対応表を置く案を2回提示したが未回答
- 10月シフト100時間削減（案は完成、未実行）
- 8/17に何が変わったかの特定

## 2026-09-14（続き）— 予約メールを全件集計。「HPB経由」の56%は広告で知っていた

オーナーが `zngifunagara@gmail.com` を接続し直したので、HPB予約通知メールの経路アンケート
（8/31開始）を**全24件**読んで集計した。氏名は転記していない。

**結論：HPBで予約した初回客16人のうち、HPBで検索して知ったのは7人（44%）。
Meta広告6人（38%）、Google3人（19%）。** 店舗システムの「HPB経由」は広告の成果を吸収している。
**広告を切れば「HPB経由」も減る。** 広告の判定を「経路別の『インスタ広告』の行だけ」で
やってきたのは過小評価だった。

方法論：初回判別はクーポン名で（`[新規]`タグは不安定）／アンケートはリピーターにも出る／
回答は自由記述で正規化が必要／【3】は検索・マップ・広告が分離不能／キャンセルが追える（16件中1件）。

- `docs/analytics/hpb-mail-survey-2026-09.md`（新規）
- `scripts/analytics/hpb-mail-tally.py`（新規・予約番号と回答のみ）
- `zenryoku-facts`・`metrics-definitions.md` を更新

## 2026-09-14（続き）— Council（5名）で広告設定を評決：維持

オーナー依頼で、5つの異なる視点の顧問が独立回答 → 匿名相互レビュー → 盲点を実データ検証 → 議長統合。
**評決：維持。Meta ¥7,200/日は9/20まで触らない。Googleは9/28まで停止のまま。**

一致：停止・減額・増額いずれも誤り／広告の採算は店内の成約率が決めている／月曜に動かすのは広告ではない。
割れた点：年齢下限25→35の復帰は「方向は正しいが時期は9/20後」で裁定。9/20後の次の1変数は新クリエイティブではなく年齢下限復帰。

検証で確定した盲点：9/20判定は週n≈5で識別不能／判定指標が広告成果を過少計上／初回で買わなかった252人は一度も戻っていない。
検証で否定：右側打ち切り（後日購入0人）／配分偏りだけで崩落を説明（30%のみ）。

限界利益を成約率10%で引き直すと ¥9,257 vs 広告費¥8,744 ＝ **+¥513**。「薄いプラス」は過大で「ほぼゼロ」が正確。

- `docs/council/ads-verdict-2026-09-14.md`（新規）

## 2026-09-14（続き）— 9/20のMeta判定Routineを「減額トリガーのみ・指標再定義版」に書き換え

Council評決の提言どおり、`trig_019WAvAngrAd559nAjDjTj6H` のプロンプトを差し替えた。
旧設計「週6人ペースで増減」は週n≈5で識別不能なため廃止。

- 判定は**減額トリガーのみ**（切替後の広告起点新規／営業日が切替前の0.5倍を下回れば ¥3,500 へ。それ以外は維持）
- 指標を**「広告起点新規」**に再定義：店舗システムのMeta系経路 ＋ 予約メールのアンケート【2】（初回・キャンセル除外）
- **累計・営業日あたり**で比較（8/28〜9/20 vs 8/1〜8/27）
- 維持なら「年齢下限25→35への復帰」を承認提案（自動実行しない）

## 2026-09-15 — 新規獲得の3ヶ月プラン（9/15〜12/14）を作成

オーナー依頼「新規が減っていることにもっとフォーカスを当て、これまで検証したデータから今後3ヶ月のプランを練る」に対応。

- 現状：新規／営業日 6.45（7月）→ 3.84（8月）→ 2.45（9月前半）。目標は12/14までに 4.0（7月水準）へ戻す
- 広告費は据え置き（月¥22万）。「広告の変更は1回に1変数」を維持
- 増分+40人／月の内訳は根拠のある4レバーのみ：予約導線の歩留まり（LINE行き止まり修正）／HPBのACR回復／紹介の回復／Googleクレジット再開（9/28以降）
- 第1月「穴を塞ぐ・計測を直す」、第2月「質を上げる・紹介を戻す」、第3月「仕上げ」の3段階と、判定カレンダー（9/20・9/28・10/14・10/22・11/1・11/14・12/14）を明記
- やらないこと（2変数同時変更・中間指標での判定・土日増員）と撤退線を明記
...
## 2026-09-25 — Metaの最適設定を評価：変更は年齢下限の1つだけ

オーナー依頼「女性比率が増えているので設定を変えるべきか。これまでの設定パターンのデータ・売上・新規販売率を全部加味して最適にしたい」。

- **女性比率の上昇は事実**（5.6→6.7→12.6→14.8%）**だが、上がり始めたのは8月で8/28の設定変更より前。**
  Meta経由の女性比率7.5%はHPB経由13.9%より低い。ただしAdvantage+ONは表示の12.6%を女性へ配る（7月実測）のでメカニズムはある。
  金額は月¥9,687と小さく、女性が買わない理由は初回特別価格が男性コースのみだから（FC第11条で是正に本部承認が要る）。**広告で女性を締め出す対処はしない**
- **推奨は年齢下限 25→35 の1変数のみ。** 20-30代10.5% vs 40代以上20.7%（p=0.0026）、20-30代の構成比は8/28以降41.9%。
  期待成約率16.4%→18.9%、限界利益/人 ¥12,338→¥13,528、**月+¥20,282。損益分岐は新規8.8%減**
- 地域・性別・Advantage+・日予算は据え置き
- **新発見：Leadイベントが来店を予測していない。** 8/28〜9/20で536 Lead→来店17人＝3.2%。
  Lead最多を理由にD_hatsujoriku へ予算78.1%（¥134,505）が寄っているが、店舗システム上のDの新規は2人（A_kibun_videoは¥32,707で12人）。
  ⚠️ ただし受付の選択肢「インスタ広告（動画）／（初上陸）」を客は区別できないため、**この配分表を根拠に予算を動かさない**。
  先に utm_content でクリエイティブ別計測を直す（広告変数ではないので1変数ルールに抵触しない）
- 7月と現在はターゲティング設定がほぼ同じ（男性25-65+・Advantage+ON）。0.94→0.71の差はクリエイティブと導線であってターゲティングではない
- 未取得：9/12以降の性別・年代（CSV再出力が要る）、8/1〜8/25のクリエイティブ別実績、7月のLead数
