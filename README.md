...
**⚠️ スキルが登録される条件（2026-08-21 実測）**

`.claude/skills/` のスキルは、**セッション開始時点でリポジトリが存在する場合にだけ**
呼び出せるスキルとして登録される。

| セッションの状態 | 結果 |
|---|---|
| リポジトリ付きで開始 | ✅ 4スキルすべて登録され `Skill(zenryoku-facts)` が動く |
| リポジトリ無しで開始し、途中で `git clone` | ❌ `Unknown skill`。`register_repo_root` を呼んでも登録されない |

そのためCoworkの定期ルーティンでスキルを使わせるには、
**そのルーティンにリポジトリを紐付けておく必要がある**（プロンプトでクローンさせても手遅れ）。
紐付いていないルーティンは、プロンプト内にルールを直書きしたまま運用する。
